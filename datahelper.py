...
class DataSet(object):
  ...
  def parse_data(self, FLAGS,  with_label=True,path=None,islog=None):
    if path is None:
      fpath = FLAGS.dataset_path	
    else:
        fpath=path
    ...
    #chemblacklist = pd.read_csv(fpath+"remove_chemicals.txt",header=None)[0]
    chems = pd.DataFrame(list(ligands.values()))[0]
    get_loc = lambda x: chems.index[chems.tolist().index(x)]
    #exclude_index = chemblacklist.apply(get_loc)

    if os.path.exists(fpath+'Y.csv'):
      Y = pd.read_csv(fpath+'Y.csv',header=None).values
    else:
      Y = pickle.load(open(fpath + "Y","rb"), encoding='latin1') ### TODO: read from raw

    if islog or FLAGS.is_log:
  ...
  def parse_real(self,with_label=True,fpath=None,):    
    print("Read %s start" % fpath)
